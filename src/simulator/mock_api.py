...
def receive_data():
    ...
    try:
        ...
        if data.get('temperature', 0) < 15:
            alerts.append(
                f"Low temperature: {data['temperature']}°C "
                f"(threshold: 15°C)"
            )
        if data.get('soil_moisture', 0) < 30:
            alerts.append(
                f"Low soil moisture: {data['soil_moisture']}% "
                f"(threshold: 30%)"
            )
        if data.get('humidity', 0) > 80:
            alerts.append(
                f"High humidity: {data['humidity']}% "
                f"(threshold: 80%)"
            )
...
def query_data():
    ...
    try:
        ...
        soils = [
            h.get('soil_moisture', 0)
            for h in history
            if h.get('soil_moisture') is not None
        ]
